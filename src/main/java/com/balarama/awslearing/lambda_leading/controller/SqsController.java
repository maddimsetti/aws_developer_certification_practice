...
public class SqsController {
	...
	public ResponseEntity<APIResponse> receiveMessages(RequestEntity<Void> request) {
        try {
            List<String> message = sqsService.receiveMessagefromSQS();
            return ResponseEntity.status(HttpStatus.OK).body(new APIResponse("success", message, "📥 Checked for messages"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<APIResponse> receiveDlqMessages(RequestEntity<Void> request) {
        try {
            List<String> message = sqsService.receiveDlqMessages();
            return ResponseEntity.status(HttpStatus.OK).body(new APIResponse("success", message, "📥 Checked for DLQ messages"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new APIResponse("error", null, e.getMessage()));
        }
